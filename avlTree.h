...
public:
	...
	*/
	...
	// Print the tree with indents
	string toString(string msg = "") const
	{
		return msg + "\n" + toString(root, "  ");
	}
	//removeMin value from the tree.
	Comparable removeMin(){
		if (isEmpty())
			//	throw new exception();
			cout << "Empty";
		Comparable rebalance;
		rebalance = removeMin(root);
		balance(root);
		return rebalance;
	}
	void balance(){
...
private:
	...
	*/
	...
	//toString which prints out the tree with indents
	...
	//removeMin remove the smallest item and rebalance
	Comparable removeMin(AvlNode* & t){
		AvlNode * temp;
		Comparable balanceValue;
		if (t->left == nullptr){
			temp = t;
			balanceValue = t->element;
			t = t->right;
			delete temp;
			balance(t);
			return balanceValue;
